...
def fisher_calculation(csv_path,threshold,large_window,short_window,sample_beginning,sample_end):
    ...
    time_array=[]
    date=[]
    global_time_since_beginning=0  #in ms

    for timestamp_treated in range(1,csv_length):
        current_timestamp=timestamp_treated
        global_time_since_beginning+=float(csv_list[timestamp_treated][2])
        heartbeat_number=0
        authorize_treatment_if_the_time_since_beginning_is_superior_to_start_time=True


        if START_TIME*1000 > global_time_since_beginning:  #Nous n'effectuons toujours pas de traitement car le temps écoulé depuis le début de l'enregistrement n'a pas dépassé START_TIME
            authorize_treatment_if_the_time_since_beginning_is_superior_to_start_time=False

        elif global_time_since_beginning<large_window and global_time_since_beginning>START_TIME*1000: #Période "intermédiaire" au début de l'algorithme durant laquelle le temps écoulé depuis le début n'a pas
                                                                                        #encore atteint la longueur de la fenêtre large, mais nous voulons quand même commencer les prédictions après 20 sec d'enregistrement
                                                                                        #Nous remplaçons donc la moyenne sur fenêtre longue par la moyenne depuis le début de l'enregistrement, jusqu'à atteindre 5 min=long_window
            short_window_hr_median,current_timestamp,global_time_since_beginning,heartbeat_number,treatment_time=calculation_intermediate_period_at_the_start_of_the_sample(csv_list,current_timestamp,global_time_since_beginning,short_window,heartbeat_number)                            
...
def calculation_intermediate_period_at_the_start_of_the_sample(csv_list,current_timestamp,global_time_since_beginning,short_window,heartbeat_number):
    treatment_time_since_beginning=0
    short_window_hr_median=0
    while treatment_time_since_beginning<=global_time_since_beginning:    #Les calculs se font tant que l'instant présent n'est pas atteint
        if current_timestamp>=len(csv_list):      
            break
        treatment_time_since_beginning+=float(csv_list[current_timestamp][2])
        if treatment_time_since_beginning >= global_time_since_beginning - short_window:    
            short_window_hr_median+=float(csv_list[current_timestamp][2])
            heartbeat_number+=1
        current_timestamp+=1
    return(short_window_hr_median,current_timestamp,global_time_since_beginning,heartbeat_number,treatment_time_since_beginning)
...
def calculation_normal(csv_list,current_timestamp,large_window,short_window,heartbeat_number):
    ...
    while treatment_time_since_beginning<=large_window:   
        ...
        if treatment_time_since_beginning >= large_window - short_window:    
            ...
            heartbeat_number+=1
        current_timestamp+=1
    return(short_window_hr_median,current_timestamp,heartbeat_number)  
