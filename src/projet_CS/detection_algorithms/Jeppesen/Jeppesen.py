...
try :
    from src.projet_CS.detection_algorithms.Jeppesen.custom_features import get_custom_features
except :
...
def jeppesen(enregistrement_path, json_path, feature_name ,sliding_window=100):
    ...
    if len(StartTime_and_EndTime_seizures)==1:  #if no seizure is detected
        StartTime_and_EndTime_seizures.append(is_timestamp_during_a_seizure[len(is_timestamp_during_a_seizure)-1][1])
